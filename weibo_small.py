...
def _read_body(obj):
    ...
    if using_gzip:
        ...
        gzipper.close()
        if fcontent:
            logging.info('gzip content uncompressed, %s bytes' % len(f_content))
        else:
            logging.warn('gzip content uncompressed, empty.')
        return fcontent
    return body
...
def _http_call(the_url, method, authorization, **kw):
    ...
    logging.info('--' * 20)
    logging.info('request to url %s, method %s' % (http_url, method))
    ...
    try:
        ...
        if hasattr(r, 'error_code'):
            logging.error('error_code=%s' % r.error_code)
            raise APIError(r.error_code, r.get('error', ''), r.get('request', ''))
        logging.info('http request ends successfully.')
        return r
    except urllib2.HTTPError, e:
        logging.error('urllib2.HTTPError')
